...
>>>>>>> Stashed changes
  ...
  </div>`}function Ve(e){m.classList.remove("is-open"),document.body.style.overflow=""}function Oe(e){e.target===e.currentTarget&&(m.classList.remove("is-open"),document.body.style.overflow="")}function _(e){e.key==="Escape"&&(m.classList.remove("is-open"),document.removeEventListener("keydown",_),document.body.style.overflow="")}let i=null,d=null;const H=/^\w+(\.\w+)?@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/;let c={email:"",comment:""};function Be(e){g.addEventListener("focus",Ue),p.addEventListener("focus",Re);const t=new FormData(f);c.email=t.get("email").trim(),c.comment=t.get("comment").trim(),g.addEventListener("blur",_e),p.addEventListener("blur",He);const s=JSON.stringify(c);localStorage.setItem(T,s),c.email&&c.comment?w.disabled=!1:w.disabled=!0}function _e(e){i=c.email;const t=e.target.closest(".small-text").querySelector(".validation");let s;g.addEventListener("click",F),i.length>45&&(s=i.slice(0,45)+"...",e.target.value=s),H.test(i)?M(t):C(t),i.length===0&&u(t)}function He(e){d=c.comment;const t=e.target.closest(".small-text").querySelector(".validation");let s;p.addEventListener("click",U),d.length>35&&(s=d.slice(0,35)+"...",e.target.value=s),d.length<1?C(t):M(t),d.length===0&&u(t)}function M(e){e.innerHTML="Success!",e.classList.add("success"),e.closest(".small-text").querySelector(".works-input").classList.add("success"),e.closest(".small-text").querySelector(".works-input").classList.remove("error")}function C(e){const t=e.dataset.input;e.innerHTML=`Invalid ${t}, try again!`,e.classList.add("error"),e.closest(".small-text").querySelector(".works-input").classList.add("error"),e.closest(".small-text").querySelector(".works-input").classList.remove("success")}function u(e){e.innerHTML="",e.classList.remove("error"),e.classList.remove("success"),e.closest(".small-text").querySelector(".works-input").classList.remove("success"),e.closest(".small-text").querySelector(".works-input").classList.remove("error")}function Fe(e){return` <div class="error-wrapper">
    <h4 class="error-title">Error!</h4>
    <p class="error-message">&#160;&#160;&#160;${e.response.data.message}</p>
  </div>`}function F(e){document.activeElement===e.target&&(e.target.value=i,H.test(i)?M(e.target):C(e.target))}function U(e){document.activeElement===e.target&&(e.target.value=d)}function Ue(e){const t=e.target.closest(".small-text").querySelector(".validation");u(t)}function Re(e){const t=e.target.closest(".small-text").querySelector(".validation");u(t)}function ze(){const e=localStorage.getItem(T);if(!e)return;const t=JSON.parse(e);f.elements.email.value=t.email,f.elements.comment.value=t.comment,t.email&&t.comment&&(w.disabled=!1)}const m=document.querySelector("#modal"),g=document.querySelector(".email-input"),p=document.querySelector(".comment-input"),w=document.querySelector(".js-works-btn"),T="feedback-form-works",f=document.querySelector(".js-works-form"),Je=document.querySelector(".js-small-email"),Ke=document.querySelector(".js-small-comment"),b=document.querySelector("#modal-error");f.addEventListener("submit",We);f.addEventListener("input",Be);document.addEventListener("DOMContentLoaded",ze);async function We(e){e.preventDefault();const t=new FormData(e.target),s={email:t.get("email"),comment:t.get("comment")};try{const o=await Z(s),n=o.title,r=o.message,v=await Ne(n,r);m.innerHTML=v,m.classList.add("is-open"),document.querySelector(".js-modal-btn").addEventListener("click",Ve),m.addEventListener("click",Oe),document.addEventListener("keydown",_),document.body.style.overflow="hidden",u(Je),u(Ke),g.value="",p.value="",w.disabled=!0,localStorage.removeItem(T),e.target.reset()}catch(o){b.classList.add("is-open"),b.innerHTML=Fe(o),setTimeout(async()=>{b.classList.remove("is-open")},5e3)}finally{g.removeEventListener("click",F),p.removeEventListener("click",U)}}
